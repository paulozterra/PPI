...
public:
    ...
    int diametro(){
        return this->calDiamtro(root);
    }

    int calDiamtro(node<T> n){
        if(!n)
            return 0;
        int ldiam = calDiamtro(n->left);
        int ddiam = calDiamtro(n->right);
        int lheight = height(n->left);
        int rheight = height(n->right);
        return max(lheight+rheight+1,max(ldiam,ddiam));
    }
    int height(node<T>n){
        if(!n)
            return 0;
    ...
    bool are_sibling(node<T> a, node<T> b){
